                            ...
                            <?php
                                ...
                                for ( $x=0; $x<($result->num_rows);$x++){
                                    ...
                                    for($q=0;$q<3;$q++){
                                        ...
                                        // New: calculate appointment remaining time
                                        $appointmentDT = new DateTime($scheduledate.' '.$scheduletime);
                                        $canBook = ($appointmentDT > $now);
                                        if($canBook){
                                            $diff = $now->diff($appointmentDT);
                                            $timeRemaining = ($diff->days>0?$diff->days.' day(s) ':'') . 
                                                             ($diff->h>0?$diff->h.' hr ':'') .
                                                             ($diff->i>0?$diff->i.' min':'');
                                        } else {
                                            $timeRemaining = "Session Closed";
                                        }
                                        ...
                                        <td style="width: 25%;">
                                                <div  class="dashboard-items search-items"  >
                                                    ...
                                                    <div style="width:100%">
                                                            ...
                                                            <div class="h4-search">
                                                                '.$scheduledate.'<br>Starts: <b>@'.substr($scheduletime,0,5).'</b><br>
                                                                ⏳ '.$timeRemaining.'
                                                            </div>
                                                            <br>';
                                        // New: disable booking if session passed
                                        if($canBook){
                                            echo '<a href="booking.php?id='.$scheduleid.'" ><button class="login-btn btn-primary-soft btn" style="width:100%">Book Now</button></a>';
                                        } else {
                                            echo '<button class="login-btn btn-disabled btn" style="width:100%" disabled>Session Closed</button>';
                                        }
