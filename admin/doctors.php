...
<body>
    ...
    function getDoctorPrivileges($id) {
        global $database;
        $priv_sql = "SELECT * FROM doctor_privileges WHERE docid=?";
        $priv_stmt = $database->prepare($priv_sql);
        $priv_stmt->bind_param("i", $id);
        $priv_stmt->execute();
        $priv_result = $priv_stmt->get_result();
        
        if($priv_result->num_rows > 0) {
            $output = "<ul class=\"privileges-list\">";
            while($priv_row = $priv_result->fetch_assoc()) {
                $output .= "<li class=\"privilege-item\">".htmlspecialchars($priv_row["privilege_name"])." (".htmlspecialchars($priv_row["permission_level"]).")</li>";
            }
            $output .= "</ul>";
        } else {
            $output = "No specific privileges assigned";
        }
        $priv_stmt->close();
        return $output;
    }
            ...
            <div id="popup1" class="overlay">
                    ...
                    <center>
                        ...
                        <table width="80%" class="sub-table scrolldown add-doc-form-container" border="0">
                            ...
                            <tr>
                                <td class="label-td" colspan="2">
                                    <label for="spec" class="form-label">Privileges: </label>
                                                                
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
                                    '.getDoctorPrivileges($id).'<br><br>
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
                                    <a href="manage-privileges.php"><input type="button" value="Manage Privileges" class="login-btn btn-primary-soft btn"></a>
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
